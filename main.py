...
model_names = model_names + sorted(name for name in models_vovnet.__dict__ 
    if name.islower() and not name.startswith("__") 
    and callable(models_vovnet.__dict__[name]))
...
def main():
    ...
    if args.pretrained:
        ...
        if 'vovnet' in args.arch:
            model = models_vovnet.__dict__[args.arch](pretrained=True)
        else:
            model = models.__dict__[args.arch](pretrained=True)
    else:
        print("=> creating model '{}'".format(args.arch))
        if 'vovnet' in args.arch:
            model = models_vovnet.__dict__[args.arch]()
        else:
    ...
    savedir = f'./checkpoint/{args.savedir}'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1, prec5, val_losses = validate(val_loader, model, criterion)
